...
def initialization():
    ...
    initial_coodinates = sample([(i1, i2) for i1 in range(0, L) for i2 in range(0, L)], n)
    ...
    for particle in range(n):
        ...
        grid[initial_coodinates[particle][0]][initial_coodinates[particle][1]] = type
...
def identify_agregate(grid, particle_type, start_pos, E_cross, vis):
    ...
    queue = [to_fixed_tuple(np.array([start_pos[0], start_pos[1]]), 2)]

    # Particles are associated with a number from which we can recover their coordinates:
    connected_particles.add(queue[0][0] * (L+1) + queue[0][1])
    ...
    while len(queue) > 0: # The loop keeps going until there is no more sites to inspect
        ...
        for d in range(6):
            ...
            neighbor_coord_1, neighbor_coord_2 = (queue[0][0]+move_list[d][0])%L, (queue[0][1]+move_list[d][1])%L
            neighbor_type = abs(grid[neighbor_coord_1][neighbor_coord_2])
            ...
            if (neighbor_type  == abs(particle_type) and E_prey != 0) or (neighbor_type != 0 and (E_cross != 0)):
                ...
                if neighbor_coord_1*(L+1) + neighbor_coord_2 not in vis and neighbor_coord_1*(L+1) + neighbor_coord_2 not in connected_particles:
                    connected_particles.add(neighbor_coord_1*(L+1) + neighbor_coord_2)
                    queue.append(to_fixed_tuple(np.array([neighbor_coord_1, neighbor_coord_2]), 2))
...
def aggregated_particles_detection(grid, particle_type, start_pos, moving_particles, directions, main_aggregate):
    ...
    if main_aggregate: # main_aggregate is True if "aggregate" only lists the particles forming the aggregate the drawn particle is part of
        ...
        for pos in aggregate: # We look at the neighbors of the particles of the aggregate in order to inspect the surroundings
            neighbors = [to_fixed_tuple(np.array([(pos // (L + 1) + m[0]) % L, (pos % (L + 1)  + m[1]) % L]), 2) for m in move_list]

            for ind in range(6):
                n_1, n_2 = neighbors[ind][0], neighbors[ind][1]
                state = grid[n_1][n_2]
                if state != 0:
                    if n_1*(L+1) + n_2 not in aggregate and n_1*(L+1) + n_2 not in new_moving_particles[ind]: # If not already considered
                        new_moving_particles = aggregated_particles_detection(grid, int(grid[n_1][n_2]), np.array([n_1, n_2]), new_moving_particles, np.array([ind]), False)
        ...
        for pos in aggregate:  # For each element of the aggregate, we inspect its surroundings in a given direction given in the arguments
            coord1, coord2 = to_fixed_tuple(np.array([(pos // (L + 1) + move_list[directions[0]][0]) % L, (pos % (L + 1) + move_list[directions[0]][1]) % L]), 2)
            site_state = grid[coord1][coord2]
            if site_state != 0:  # if not empty
                if coord1 * (L + 1) + coord2 not in new_moving_particles[directions[0]]:  # If not already considered
                    new_moving_particles = aggregated_particles_detection(grid, site_state, np.array([coord1, coord2]), new_moving_particles, directions, False)
...
def main(initial_state_of_the_lattice):
    ...
    L: Integer
        size of the system (length of an edge of the whole lattice)
    ...
    while t <= tmax:
        ...
        particle_type = grid_state[random_site[0]][random_site[1]]

        free_neighboring_sites =  np.array([dir for dir in range(6) if grid_state[(random_site[0] + move_list[dir][0]) % L][(random_site[1] + move_list[dir][1]) % L] == 0])
        number_of_free_neighbors = len(free_neighboring_sites)
        occupied_neighbors = [[(random_site[0] + move_list[dir][0]) % L, (random_site[1] + move_list[dir][1]) % L] for dir in range(6) if grid_state[(random_site[0] + move_list[dir][0]) % L][(random_site[1] + move_list[dir][1]) % L] != 0]


        # Determination of the rates of the events that can potentially occur in the site we've drawn
        # The multiplication by L^2 accounts for the fact that, if we randomly draw a site and then try to make an event x to occur there,
        # the overall probability is k_x and the probability given that we draw the right site k_x*L^2
        if particle_type == 1:
            energy, killing_proba, lysis_proba = E_prey, k_kill*L**2, 0
            same_type_neighbors = np.sum(np.array([1 for n in occupied_neighbors if abs(grid_state[n[0]][n[1]]) == 1]))
            neighboring_predators = np.sum(np.array([1 for n in occupied_neighbors if grid_state[n[0]][n[1]] == 2]))
        elif particle_type ==  - 1:
            energy, killing_proba, lysis_proba = E_prey, 0, k_lysis*L**2
            same_type_neighbors = np.sum(np.array([1 for n in occupied_neighbors if abs(grid_state[n[0]][n[1]]) == 1]))
            neighboring_predators = np.sum(np.array([1 for n in occupied_neighbors if grid_state[n[0]][n[1]] == 2]))
        elif particle_type == 2:
            energy, killing_proba, lysis_proba = E_predator, 0, 0
            same_type_neighbors = np.sum(np.array([1 for n in occupied_neighbors if abs(grid_state[n[0]][n[1]]) == 2]))
            neighboring_predators = 0
        if number_of_free_neighbors > 0:
            hopping_proba = number_of_free_neighbors*k_hop*L**2*np.exp(-energy*same_type_neighbors - E_cross*(6-number_of_free_neighbors-same_type_neighbors))
            if particle_type > 0:
                division = k_d*L**2
            else:
                division = 0
        ...
        if  random_event <= lysis_proba:
            grid_state[random_site[0]][random_site[1]] = 0
        ...
        # Diffusion
            ...
            new_coordinates = [(random_site[0] + move_list[random_direction][0]) % L, (random_site[1] + move_list[random_direction][1]) % L]
            grid_state[new_coordinates[0]][new_coordinates[1]] = particle_type
            grid_state[random_site[0]][random_site[1]] = 0
        ...
        # Aggregate diffusion
            ...
            overall_connected_particles = [[nb.float32(0)] for _ in range(6)]
            for m in range(6):
            ...
            moving_particles_list += aggregated_particles_detection(grid_state, particle_type, random_site, overall_connected_particles, np.arange(6), True)


            aggregate_hopping_rates = np.zeros(6)
            for ele in range(len(moving_particles_list)):
                if len(moving_particles_list[ele]) > 1:
                    aggregate_hopping_rates[ele] += k_hop * L ** 2 / len(moving_particles_list[5-ele]) # We divide by the number of particles
            ...
            if  lysis_proba + division + hopping_proba + killing_proba * neighboring_predators < random_event <= lysis_proba + division + hopping_proba + killing_proba * neighboring_predators + np.sum(aggregate_hopping_rates):
                ...
                coordinates = [[int(particle // (L+1)), int(particle % (L+1))] for particle in moving_particles_list[choice]]
                new_coordinates_list = [[(coordinates[particle][0] + move_list[choice][0]) % L, (coordinates[particle][1] + move_list[choice][1]) % L] for particle in range(len(coordinates))]

                all_types = []
                # We remove the particles that move and relocate them
                for site in coordinates:
                    all_types.append(int(grid_state[site[0]][site[1]]))
                    grid_state[site[0]][site[1]] = 0
                for particle in range(len(coordinates)):
                    grid_state[new_coordinates_list[particle][0]][new_coordinates_list[particle][1]] = all_types[particle]
...
if __name__ == "__main__":
    ...
    # Arguments
    L = 100 # With a (2D) triangular lattice, the system is a Parallelogram with edges of size L
    ...
    c = (k_d + 6 * k_hop) * L ** 2 / 0.99 # Given that during an iteration we draw an isolated lysing cell,
    ...
    # Given the primitive translation vectors of the triangular lattice, "move_list" gives, from a site (0, 0, 0), where are its neighbors
